%       **********************
%       ANALYSE D'UN SQUELETTE Version V1.11
%       **********************
% En ENTREE :
...
while indpil ~= 0
    ...
    switch etat
        ...
        case 4 % P = (i,j) est un carrefour
            ...
            % [numsom,qq] = sommet(i,j,icar,jcar); V1.9
            [numsom,qq] = carrefour (i,j,icar,jcar,nbrecar); % V1.10
            % numsom est le numero du carrefour P = (i,j)
        ...
        case 2 % P = (i,j) pixel "complexe" , voisin d'un carrefour
            ...
            if ~carrefour_8_voisin && ~simple_voisin
                ...
                for k =1:8
                    iq = i + itab(k);
                    jq = j + jtab(k);
                    % [numsom,carrefour_dejarencontre] =
                    % sommet(iq,jq,icar,jcar); version v1.9
                    [numsom,carrefour_dejarencontre] = carrefour(iq,jq,icar,jcar,nbrecar); %V1.10
        ...
        case {0,1} % P = (i,j) pixel "simple" du squelette
            ...
            delta = gamma - alpha;
            teta = beta + beta;
            ub = delta + sqrt(double(delta * delta + teta * teta));
            ...
            distq = abs(ua * double(jq) - ub * double(iq) + uc) / sqrt(double(ua * ua + ub * ub));
            if distq >= 1.1 % 0.51 dans la V1.10
                ...
                if im(iq,jq) == 2
                    ...
                    for k = 1:8
                        ...
                        if im(ir,jr) ~= 7 && (ir ~= i || jr ~= j)
                            ...
                            distr = abs(ua * double(jr) - ub * double(ir) + uc) / sqrt(double(ua * ua + ub * ub));
                            if  distr > distq
