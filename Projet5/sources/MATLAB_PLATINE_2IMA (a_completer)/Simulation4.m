...
%
xy = zeros(3,subd+1,nombre_objets); % xy(1:2,:,i) : echantillons du chemin i
xy2 = zeros(2,subd+1);
s1 = zeros(1,nombre_objets); % s1(i) : longueur du chemin i
%
% Tableau des collisions
nbrecollision = zeros(nbremaxobj,1); % nbrecollision(k)= nombre total de collisions de l'objet 1 avec l'objet k lors de la simulation
collision = zeros(nombre_objets,nombre_objets,3,nbremaxcollision); 
% collision (1,k,1,nocollision) = abscisse du lieu de la nocollision-ieme de
% l'objet 1 avec l'objet k
% collision (1,k,2,nocollision) = ordonnee du lieu de la nocollision-ieme de
% l'objet 1 avec l'objet k
% collision (1,k,3,nocollision) = instant de la nocollision-ieme de
% l'objet 1 avec l'objet k
%
hdepl = figure ('Name',['Simulation ',NOMSCENE],'Position',[left,bottom,width,length1]); %creation nouvelle figure
imagesc(im);
hold on
%
...
for i = 1:nombre_objets
    if strcmp(T(i).chemin,'DROITE')
        ...
        jarr = T(i).arrivee(2);
        %
        for p=1:subd+1 % au lieu de p=1:subd version 1.9
            mu = (p-1)/subd;
        ...
        if strcmp(T(i).chemin,'NURBS')
            ...
            nurbsf = T(i).nurbs;
            xy(:,:,i) = nrbeval(nurbsf,linspace(0.0,1.0,subd+1)); % subd + 1 au lieu de subd version 1.9
            xy2 = xy(1:2,:,i); %inutile
            nrbplot(nurbsf,subd+1,T(i).couleur);
        else
            disp('ERREUR');
        end       
    end
...
for i=1:nombre_objets
    ...
    P(i).s = s;
    %
    tempsparcours(i) = nbrerepetition*send/v; % temps de parcours de l'objet i (si pas de collision)
    % version 1.9 : ceil(nbrerepetition*send/v) au lieu de length(s)
...
while t <= maxtempsparcours % Boucle temporelle avec maxtempsparcours variable à cause des collisions éventuelles
    ...
    for k = 1:nombre_objets
        ...
        xunit0 = double(xcentre) + rrob(k) * cos(th);
        yunit0 = double(ycentre) + rrob(k) * sin(th);
        xunit = [xunit xunit0];
        yunit = [yunit yunit0];
    end
    ...
    for kobj = 2:nombre_objets
        ...
        sumray = rrob(1)+rrob(kobj);
         if distance < sumray && Vulnerable(1) && Vulnerable(kobj)
            ...
            %
            nbrecollision(kcol) = nbrecollision(kcol) + 1; % une collision de plus de l'objet 1 avec l'objet kcol
            %
            for k=1:2
                collision(1,kcol,k,nbrecollision(kcol)) = xy(k,ii(1),1); % position de l'objet 1 quand il entre en collision avec l'objet kcol
                collision(kcol,1,k,nbrecollision(kcol)) = xy(k,ii(kcol),kcol); % position de l'objet kcol quand il entre en collision avec l'objet 1
            end
            collision(1,kcol,3,nbrecollision(kcol)) = t; % instant de la collision de l'objet 1 avec l'objet kcol
            collision(kcol,1,3,nbrecollision(kcol)) = t; % instant de la collision de l'objet kcol avec l'objet 1
            %
...
disp(' ');
%% Sauvegarde Scene avec les informations sur les collisions
for i = 1:nombre_objets
    T(i).collision = collision(i,:,:,:);
end
T(1).nbrecollision = nbrecollision;
